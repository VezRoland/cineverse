...
interface ExtendedTV extends TV {
  media_type?: MediaType;
}
...
export const CardGrid = ({
  ...
  return (
    <section className="w-full grid grid-cols-[repeat(auto-fit,minmax(theme(width.44),1fr))] auto-rows-fr gap-4">
      {items.map((media) =>
        [media.media_type, mediaType].includes('movie') ? (
          <MediaCard
            ...
            rating={
              Math.round(((media as Movie).vote_average / 2) * 10) /
              10
            }
          ...
          <MediaCard
            ...
            id={media.id}
            rating={
              Math.round(((media as TV).vote_average / 2) * 10) / 10
            }
      ...
      {items.length < 4
        ? new Array(4 - items.length)
            .fill(null)
            .map(() => <div key={crypto.randomUUID()}></div>)
        : null}
    </section>
  );
};
