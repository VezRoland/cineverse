...
interface ExtendedSeason extends Season { vote_average: number, vote_count: number }

export const loader = async ({ params }: LoaderFunctionArgs) => {
  if (!params.id) return null

  try {
    const tvShow = await tmdb.tvShows.details(parseInt(params.id), [ "videos", "credits", "reviews", "episode_groups" ])
    return tvShow
  } catch {
    return null
  }
...
export default function TVShowPage() {
  ...
  if (!tvShow) {
    return (
      <main className="flex-1 grid place-items-center">
        <h1 className="text-4xl font-semibold">TV show can&apos;t be found!</h1>
      </main>
    )
  }
  ...
  return (
    <>
      <Header
        title={name}
        breadcrumbRoutes={[[ "Home", "/" ], [ "TV Shows", "/tv-shows" ], [ name, "" ]]}
      />
      <main className="w-full flex-1 flex flex-col gap-8">
        <Card className="relative bg-transparent bg-gradient-to-b from-background to-transparent">
          <CardHeader className="flex-row flex-wrap justify-between items-center gap-4 space-y-0">
            <CardTitle>
              <h2 className="text-2xl font-semibold text-primary">About this TV show</h2>
            </CardTitle>
            <ul className="flex flex-wrap gap-2 overflow-auto">
              {genres.map(genre => (
                <Button
                  ...
                  asChild
                >
                  <Link className="text-xs" to={`/tv-shows?genres=${genre.id}`}>
                    {genre.name}
                  </Link>
                </Button>
              ))}
            ...
            <ul className="flex flex-wrap gap-2">
              ...
              <li>
                <Badge className="w-max flex gap-2" variant="outline">
                  ...
                  {new Date(first_air_date).toLocaleDateString("hu-HU")} - {status === "Ended" ? new Date(last_air_date).toLocaleDateString("hu-HU") : ""}
                </Badge>
              </li>
            </ul>
          </CardFooter>
          ...
          <TableBody>
            {seasons.map(season => (
              <TableRow key={season.id}>
                ...
                <TableCell>{season.episode_count ? season.episode_count : "TBA"}</TableCell>
                <TableCell>{season.air_date ? new Date(season.air_date).toLocaleDateString("hu-HU") : "TBA"}</TableCell>
                <TableCell className="flex flex-wrap gap-2">
                  <span>{Math.round(((season as ExtendedSeason).vote_average / 2) * 10) / 10} / 5</span>
                  <Rating rating={Math.round(((season as ExtendedSeason).vote_average / 2) * 10) / 10} />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
